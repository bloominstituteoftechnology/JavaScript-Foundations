...
*/
function mCalculator(principal, interestRate, years){
    let monthlyInterestRate = (interestRate / 12);
    let periods = (years * 12)
    let n1 = Math.pow ((1+ monthlyInterestRate), periods);
    let numerator = (principal * n1 * monthlyInterestRate);
...
console.log(mCalculator(200_000, 0.05, 30))
...
*/
function mortCalculator(principal, interestRate, years, credit){
    let monthlyInterestRate = (interestRate / 12);
    let periods = (years * 12)
    let n1 = Math.pow ((1+ monthlyInterestRate), periods);
    let monthlyRate = (principal * n1 * monthlyInterestRate) / ( n1 - 1);
    if (credit > 740) {
        monthlyRate * 0.95
    } else if (credit < 660) {
        monthlyRate * 1.05
...
console.log(mortCalculator(200_000, 0.05, 30, 620))
...
// 🏡 Task 6: Loops
/* Write a new function called variableInterestRate. This function should be the same as mortgageCalculator, except it should console.log the monthly payment for 
10 different interest rates at 0.5% increments plus or minus 2% from the inputted interest rate. Complete these calculations using a for loop.
